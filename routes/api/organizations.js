...
//@route POST api/organizations/:org_urlname/products/create_csv
//@desc  Create product from csv
//@acces Private
router.post(
  "/:org_urlname/products/create_csv",
  passport.authenticate("jwt", { session: false }),
  (req, res) => {
    const errors = {};

    var fileName = req.body.fileName
    var data = csvToJson.fieldDelimiter(';') .getJsonFromCsv(fileName);
    ...
    Organization.findOne({ urlName: req.params.org_urlname })
      .then(organization => {
        ...
        // Product creation
        for (let obj of data) {
          let { errors, isValid } = validateCreateProductOrServiceInput(obj);
          // Check input validation
          if (!isValid) {
            return res.status(400).json(errors);
          }
          Subproject.findById(obj.subprojectId).then(subproject => {
            if (!subproject) {
              errors.subproject = "Subproject not found";
              return res.status(404).json(errors);
            }
            ...
            productFields.name = obj.name;
            productFields.price = obj.price;
            productFields.description = obj.description;
            productFields.organization = organization._id;
            productFields.subprojectId = obj.subprojectId;

            if (req.body.tags) productFields.tags = obj.tags.split(",");
            if (req.body.image) productFields.image = obj.image;
            if (req.body.innerId) productFields.innerId = obj.innerId;
            if (req.body.potencial) productFields.potencial = obj.potencial;
            ...
            newProduct
              ...
              // Add product to organization list
              addAsperantoType = "Производитель";
              organization.productsList.unshift(newProduct);
